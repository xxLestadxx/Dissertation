...
describe('Starting tests for ssidentity', function() {
  ...
  it('network-name should be ssidentity@0.0.2-deploy.40',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('ssidentity@0.0.2-deploy.40.bna');
    });
  });
  ...
  it('navbar-brand should be ssidentity',() => {
    ...
    it('Person table should have 9 columns',() => {
      page.navigateTo('/Person');
      element.all(by.css('.thead-cols th')).then(function(arr) {
    ...
    it('HighSchool table should have 7 columns',() => {
      page.navigateTo('/HighSchool');
      element.all(by.css('.thead-cols th')).then(function(arr) {
        expect(arr.length).toEqual(7); // Addition of 1 for 'Action' column
      });
    ...
    it('DrivingSchool table should have 5 columns',() => {
      page.navigateTo('/DrivingSchool');
      element.all(by.css('.thead-cols th')).then(function(arr) {
        expect(arr.length).toEqual(5); // Addition of 1 for 'Action' column
      });
    ...
    it('University table should have 7 columns',() => {
      page.navigateTo('/University');
      element.all(by.css('.thead-cols th')).then(function(arr) {
        expect(arr.length).toEqual(7); // Addition of 1 for 'Action' column
      });
