...
describe('Starting tests for airlinev8', function() {
  ...
  it('website title should be airlinev8', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('airlinev8');
    })
  });

  it('network-name should be airlinev8@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('airlinev8@0.0.1.bna');
    });
  });

  it('navbar-brand should be airlinev8',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('airlinev8');
    });
  });
